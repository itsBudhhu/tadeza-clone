...
def mute(update, context):
    ...
    if user_id in [777000, 1087968824]:
        message.reply_text(
            f"{str(user_id)} is an account reserved for telegram, I cannot mute it!"
        )
    ...
    if member := chat.get_member(int(user_id)):
        if is_user_admin(chat, user_id, member=member):
            ...
            reply_msg = f"*{member.user.first_name}* (`{member.user.id}`) has been muted in *{chat.title}*."
...
def temp_mute(update, context):
    ...
    try:
        if member.can_send_messages is None or member.can_send_messages:
            ...
            reply_msg = f"*{member.user.first_name}* (`{member.user.id}`) has been muted in *{chat.title}* for {time_val}!."
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text(f"shut up! muted for {time_val}!", quote=False)
            return log
        else:
